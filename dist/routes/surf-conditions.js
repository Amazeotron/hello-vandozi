...
const jsonUrl = "https://services.surfline.com/kbyg/regions/forecasts/conditions?subregionId=5cc73566c30e4c0001096989&days=1&accesstoken=b892cc4f756bdbce41c7abfd05f96cae384664fd";
const endpoint = "https://production-sfo.browserless.io/chromium/bql";
export async function fetchSurfReport() {
    // First try and just load the JSON
    const jsonReport = await fetch(jsonUrl, {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
        },
    });
    const jsonData = (await jsonReport.json());
    const observation = jsonData.data.conditions[0]?.observation;
    if (observation) {
        return {
            type: "text",
            body: `The current surf conditions at Linda Mar North are: ${observation}`,
        };
    }
    const response = await fetch(`${endpoint}?token=${process.env.BROWSERLESS_TOKEN}`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
      ...
      mutation GetSurfReport($url: String!) {
        ...
        selector: text(selector: "[class^='CurrentWaveRating_rating']") {
          text
        }
      }`,
            variables: {
...
export const surfConditions = async (req, res) => {
    ...
    const audio = await synthesizeText(surfData.body);
